...
public class APIWebMvcConfigurer implements WebMvcConfigurer {
    ...
    public void addCorsMappings(CorsRegistry registry) {
        ...
        registry.addMapping("/api/**")  // CORS를 적용할 URL 패턴 지정, '/api/**'는 '/api/'로 시작하는 모든 경로 포함
                .allowedOrigins("http://example.com", "http://localhost:3000", "http://localhost:8083")  // 허용할 원점(도메인) 지정, 여기서는 example.com과 localhost를 허용
                .allowedMethods("GET", "POST", "PUT", "DELETE")  // 허용할 HTTP 메서드 지정, 기본적인 CRUD 작업에 사용되는 메서드 포함
